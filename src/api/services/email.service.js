...
const sendResetPasswordEmail = async (to, token) => {
  ...
  const resetPasswordUrl = `http://${config.url}:${config.port}/v1/auth/reset-password?token=${token}`;
...
const sendEmailActivationEmail = async (to, token) => {
  ...
  const activationUrl = `http://${config.url}:${config.port}/v1/auth/verify-email?token=${token}`;
  const text = `Dear user, 
